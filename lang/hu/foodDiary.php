<?php

return [
    "remaining_label" => "Hátralévő",
    "date" => "Ételnapló dátuma",
    "search_title" => "Keresés",
    "search_placeholder" => "Keress név alapján ételt.",
    "paginator_visible_range" => "Látszik {first}-től {last}-ig a {total}-ból",
    "paginator_page_of" => "{page}. oldal a {pageCount}-ból",
    "selected_food_title" => "Kiválasztott étel",
    "selected_food_subtitle" => "Add hozzá a kiválasztott dátumhoz.",
    "no_selected_food" => "Nincs kiválasztott étel.",
    "meal_type_label" => "Étkezés típusa",
    "meal_type_placeholder" => "Válassz étkezést",
    "amount_label" => "Mennyiség",
    "unit_label" => "Mértékegység",
    "add_button" => "Hozzáadás",
    "added_response" => "Hozzáadtad: {food} ({meal}) x{amount} — {date}",
    "create_food_title" => "Hiányolsz valamit?",
    "food_name_label" => "Név",
    "calorie_label" => "Kalória",
    "fat_label" => "Zsír",
    "carb_label" => "Szénhidrát",
    "protein_label" => "Fehérje",
    "note_label" => "Megjegyzés",
    "image_optional" => "Kép (opcionális)",
    "choose_image" => "Kép kiválasztása",
    "save_food" => "Étel mentése",
    "diary_title" => "Ételnapló",
    "meal_label" => "étkezés",
    "delete" => "Törlés",
    "no_entries" => "Nincs bejegyzés erre a dátumra.",
    "success_title" => "Siker",
    "food_saved_success" => "Az étel sikeresen mentve lett.",
    "dialog_close" => "Bezárás",
    "response_dialog_title" => "Napló",
    "confirmation" => "Megerősítés",
    "add_new_food" => "Adj új ételt az adatbázishoz",
    "delete_confirmation" => "Biztosan törölni akarod ezt az ételt?",
    "breakfast" => "Reggeli",
    "lunch" => "Ebéd",
    "dinner" => "Vacsora",
    "snack" => "Nasi",
    "other" => "Más",
    "kcal_unit" => "kcal",
    "protein_short" => "F",
    "carb_short" => "SZ",
    "fat_short" => "ZS",
    "visuals_title" => "Megjelenés",
    "visuals_subtitle" => "Az étel előnézete",
    "no_image_label" => "Nincs kép",
    "food_name_placeholder" => "pl. Görög joghurt",
    "protein_label_with_unit" => "FEHÉRJE (G)",
    "carb_label_with_unit" => "SZÉNHIDRÁT (G)",
    "fat_label_with_unit" => "ZSÍR (G)",
    "image_label" => "Étel fotók",
    "add_replace_photo" => "Fotó hozzáadása vagy cseréje",
    "product_details" => "Termék részletei",
    "add_to_diary_title" => "Hozzáadás a naplóhoz",
    "add_to_diary_subtitle" => "Válaszd ki az étkezést és az adagot.",
    "calculated_totals" => "Számított összesen",
    "meal_log_title" => "Étkezési napló",
    "activity_food" => "Tápanyagbevitel",
    "activity_workout" => "Fizikai aktivitás",
    "update" => "Ételnapló frissítése",
];