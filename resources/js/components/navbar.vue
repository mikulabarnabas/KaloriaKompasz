...
<template>
  <header class="sticky top-0 z-50 border-b border-neutral-border bg-background-dark/80 backdrop-blur">
    <div class="max-w-7xl mx-auto px-4 md:px-6 py-4 flex justify-between items-center">
      ...
      <div class="flex items-center gap-2 md:gap-3 cursor-pointer shrink-0 flex-1 group" @click="router.visit('/')">
        <IconCompass class="size-10 text-green-600 group-hover:scale-110 transition-transform" />
      ...
      <nav class="hidden lg:flex gap-8 text-lg font-medium text-main-text justify-center">
        <button @click="router.visit('/fdiary')" class="hover:text-primary transition-colors cursor-pointer">{{ $t('navbar.foodDiary') }}</button>
        <button @click="router.visit('/wdiary')" class="hover:text-primary transition-colors cursor-pointer">{{ t('navbar.workoutDiary') }}</button>
        <button @click="router.visit('/stats')" class="hover:text-primary transition-colors cursor-pointer">{{ t('navbar.stats') }}</button>
      </nav>
      ...
      <div class="flex items-center gap-1 sm:gap-2 flex-1 justify-end">
        ...
        <button @click="mobileMenuOpen = !mobileMenuOpen"
          class="size-10 flex lg:hidden items-center justify-center text-main-text cursor-pointer hover:bg-white/5 rounded-full transition-colors">
    ...
    >
      <div v-if="mobileMenuOpen"
        class="lg:hidden absolute top-full left-0 w-full bg-background-dark border-b border-neutral-border shadow-xl px-6 py-8 space-y-6 flex flex-col items-center">
        
        <button @click="navigate('/fdiary')" class="text-lg font-medium text-main-text hover:text-primary w-full cursor-pointer text-center py-2">
          {{ t('navbar.foodDiary') }}
        </button>
        
        <button @click="navigate('/wdiary')" class="text-lg font-medium text-main-text hover:text-primary w-full cursor-pointer text-center py-2">
          {{ t('navbar.workoutDiary') }}
        </button>
        
        <button @click="navigate('/stats')" class="text-lg font-medium text-main-text hover:text-primary w-full cursor-pointer text-center py-2">
          {{ t('navbar.stats') }}
        </button>
      </div>
    </Transition>
