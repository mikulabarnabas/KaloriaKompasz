...
<template>
    <AppLayout>
        <div
            class="relative min-h-[75vh] bg-background-dark flex items-center justify-center px-6 py-24 overflow-hidden">
            <div class="relative z-10 w-full max-w-md space-y-6 animate-fly-in">
                <div
                    class="bg-neutral-dark p-8 md:p-10 rounded-3xl shadow-2xl border border-neutral-border/10 dark:border-neutral-border/30">
                    ...
                    <form class="space-y-5" @submit.prevent="onSubmit" novalidate>
                        <InputField v-model="form.email" type="email" :label="t('auth.email')"
                            :error="form.errors.email" readonly class="opacity-60 pointer-events-none" />

                        <InputField v-model="form.password" type="password" :label="t('auth.new_password')"
                            :error="form.errors.password" autocomplete="new-password" placeholder="••••••••"
                            @change="form.validate('password')" />

                        <InputField v-model="form.password_confirmation" type="password"
                            :label="t('auth.password_confirmation')" :error="form.errors.password_confirmation"
